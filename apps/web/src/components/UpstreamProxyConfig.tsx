...
export function UpstreamProxyConfig({ isOpen, onClose }: UpstreamProxyConfigProps) {
  ...
  return (
    <Modal
      ...
      titleIcon={<Globe className="w-5 h-5 text-accent" aria-hidden="true" />}
      size="md"
      footer={footer}
    >
      <div className="p-4">
        {loading ? (
          ...
          <div className="space-y-4">
            {error && (
              <div
                className="p-3 rounded-md bg-danger/12 border border-danger/25 text-danger text-sm"
                role="alert"
              >
                {error}
              </div>
            ...
            <div>
              <label htmlFor={typeId} className="block text-sm font-medium text-ink mb-1">
                Proxy Type
              </label>
              <Select
                id={typeId}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <label htmlFor={hostId} className="block text-sm font-medium text-ink mb-1">
                  Host
                </label>
                <Input
                  id={hostId}
              ...
              <div>
                <label htmlFor={portId} className="block text-sm font-medium text-ink mb-1">
                  Port
                </label>
                <Input
                  id={portId}
            ...
            <div className="border-t border-edge pt-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-ink">Authentication</span>
              ...
              {useAuth && (
                <div className="space-y-3">
                  <div>
                    <label htmlFor={usernameId} className="block text-sm text-ink-secondary mb-1">
                      Username
                    </label>
                    <Input
                      id={usernameId}
                  ...
                  <div>
                    <label htmlFor={passwordId} className="block text-sm text-ink-secondary mb-1">
                      Password
                    </label>
                    <Input
                      id={passwordId}
            ...
            <div className="border-t border-edge pt-4">
              <label htmlFor={bypassId} className="block text-sm font-medium text-ink mb-2">
              ...
              <div className="flex flex-wrap gap-2">
                {bypassRules.map((rule) => (
                  <span
                    key={rule}
                    className="inline-flex items-center gap-1 px-2 py-1 bg-surface-raised border border-edge rounded text-sm text-ink-secondary"
                  >
                    {rule}
                    <button
                      type="button"
