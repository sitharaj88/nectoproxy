...
export function MobileNav({ activeTab, onTabChange, breakpointCount = 0 }: MobileNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-surface-raised border-t border-edge z-40 sm:hidden">
            ...
            >
              <div className="relative">
                ...
                {tab.badge && tab.badge > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-danger text-white text-[10px] font-medium rounded-full flex items-center justify-center">
              ...
              {isActive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-12 h-0.5 bg-accent rounded-full" />
              )}
            </button>
          );
        })}
