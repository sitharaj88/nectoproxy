...
import { Button, Input, Switch, IconButton, Badge, cn } from './ui';
import {
  X,
  Plus,
  Trash2,
      ...
      <>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          ...
          {throttleRules.length > 0 && (
            <div>
              <h4 className="text-xs font-medium uppercase tracking-wide text-ink-muted mb-3">Active Throttle Rules</h4>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-lg border transition-colors ${
                        rule.enabled
                          ? 'bg-canvas border-edge-subtle'
                          : 'bg-canvas/50 border-edge-subtle opacity-60'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        {/* Action Badge */}
                        <Badge tone={rule.action === 'throttle' ? 'danger' : 'warn'}>
          ...
          {throttleRules.length === 0 && !showAddForm && (
            <div className="text-center py-8">
              ...
              <p className="text-ink-secondary mb-1">No throttle rules configured</p>
          ...
          {/* Add Rule Form */}
          {showAddForm && (
            <div className="border border-edge rounded-lg p-4 space-y-4 bg-canvas/50">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-medium text-ink">New Throttle Rule</h4>
                <IconButton
                  label="Close form"
              ...
              {/* Error message */}
              {error && (
                <div className="p-2 bg-danger/12 border border-danger/25 text-danger text-sm rounded">
                  {error}
                </div>
              )}
              ...
              {/* Rule Type Toggle */}
              <div>
                <label className="block text-sm text-ink-secondary mb-2">Rule Type</label>
                <div className="flex gap-2">
                  <button
                    onClick={() => setRuleType('throttle')}
                    className={cn(
                      'flex-1 py-2 px-3 text-sm rounded-md transition-colors border',
                      ruleType === 'throttle'
                  ...
                  <button
                    ...
                    className={cn(
                      'flex-1 py-2 px-3 text-sm rounded-md transition-colors border',
                      ruleType === 'delay'
                        ? 'bg-warn/12 border-warn/40 text-warn'
                        : 'bg-surface-raised border-edge text-ink-secondary hover:bg-surface-overlay'
                    )}
                  >
              ...
              {ruleType === 'throttle' ? (
                <>
                  {/* Preset Selector */}
                  <div>
                    <label className="block text-sm text-ink-secondary mb-1">Network Preset</label>
                    <div className="relative">
                      <button
                        onClick={() => setShowPresetDropdown(!showPresetDropdown)}
                        className="w-full flex items-center justify-between bg-surface-raised border border-edge rounded-md px-3 py-2 text-sm text-ink hover:bg-surface-overlay hover:border-edge-strong focus:outline-none focus:border-accent"
                      >
                        <div className="flex items-center gap-2">
                          {getPresetIcon(selectedPreset.icon)}
                          <span>{selectedPreset.name}</span>
                          <span className="text-ink-muted">- {selectedPreset.description}</span>
                        </div>
                        <ChevronDown className="w-4 h-4 text-ink-muted" />
                      </button>

                      {showPresetDropdown && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-surface-overlay border border-edge rounded-md shadow-popover z-10 py-1">
                          {THROTTLE_PRESETS.map((preset) => (
                            <button
                              key={preset.name}
                              onClick={() => handlePresetSelect(preset)}
                              className={cn(
                                'w-full flex items-center gap-3 px-3 py-2 text-sm text-ink hover:bg-surface-raised text-left',
                                selectedPreset.name === preset.name && 'bg-surface-raised'
                              )}
                            >
                              {getPresetIcon(preset.icon)}
                              <span className="font-medium">{preset.name}</span>
                              <span className="text-ink-secondary text-xs">{preset.description}</span>
                              {preset.latency > 0 && (
                                <span className="text-ink-muted text-xs ml-auto">+{preset.latency}ms</span>
                              )}
                            </button>
                          ))}
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm text-ink-secondary mb-1">Bandwidth</label>
                      <div className="flex">
                        <Input
                          type="number"
                          sizeVariant="md"
                        ...
                        <select
                          ...
                          disabled={selectedPreset.name !== 'Custom'}
                          className="bg-surface-overlay border border-l-0 border-edge rounded-r-md px-2 text-sm text-ink disabled:opacity-50 focus:outline-none"
                        >
                  ...
                  {activeBytesPerSecond > 0 && (
                    <div className="bg-surface-raised/50 border border-edge-subtle rounded-md p-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        min="0"
                      />
                      <p className="text-xs text-ink-muted mt-1">Random jitter +/- this value</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-surface-raised/50 border border-edge-subtle rounded-md p-3">
                    <div className="flex items-center gap-2 text-xs text-ink-secondary">
                      ...
                      <span>
                        ...
                        <span className="text-warn font-medium">{delayMs}ms</span>
                        {customVariance > 0 && (
                          <span> (+/- <span className="text-warn">{customVariance}ms</span>)</span>
                        )}
                      </span>
                    </div>
                  </div>
          ...
          {!showAddForm && throttleRules.length > 0 && (
            <button
              onClick={() => setShowAddForm(true)}
              className="w-full flex items-center justify-center gap-2 py-2.5 border border-dashed border-edge-strong rounded-lg text-sm text-ink-secondary hover:text-ink hover:border-accent transition-colors"
            >
