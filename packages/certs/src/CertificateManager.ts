...
// Bump when the leaf-cert format changes so stale cached certs are regenerated.
// v2: fixed the Authority Key Identifier to reference the CA's SKI (chain fix).
const CERT_CACHE_VERSION = '2';
...
export class CertificateManager {
  ...
  async initialize(): Promise<void> {
    ...
    // Discard cached leaf certs generated by an older, incompatible format.
    await this.migrateCertCache();
  ...
  private async migrateCertCache(): Promise<void> {
    const domainsDir = path.join(this.config.certsDir, 'domains');
    const versionFile = path.join(domainsDir, '.cache-version');

    let current = '';
    try {
      current = (await fs.promises.readFile(versionFile, 'utf-8')).trim();
    } catch {
      // No version file — pre-versioning cache or first run.
    }

    if (current === CERT_CACHE_VERSION) return;

    // Format changed (or upgrade from an unversioned cache): remove stale leaf
    // certs so they regenerate with the current format on next request.
    try {
      const files = await fs.promises.readdir(domainsDir);
      await Promise.all(
        files
          .filter((f) => f !== '.cache-version')
          .map((f) => fs.promises.unlink(path.join(domainsDir, f)).catch(() => {}))
      );
    } catch {
      // Directory may not exist yet.
    }
    await fs.promises.writeFile(versionFile, CERT_CACHE_VERSION);
  }
